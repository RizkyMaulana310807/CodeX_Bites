<nav x-data="{ open: false, cartOpen: false }" class="bg-white shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="flex items-center">
                <!-- Icon Keranjang -->
                <button @click="cartOpen = !cartOpen" class="p-2 rounded-full text-gray-600 hover:text-[#3E2723] hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                ...
                <div class="md:hidden ml-2">
                    <button @click="open = !open" class="p-2 rounded-full text-gray-600 hover:text-[#3E2723] hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
    ...
    <div x-show="cartOpen" @click.away="cartOpen = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95">
        <div class="py-2 px-4">
            <p class="text-sm text-gray-700">Keranjang belanja kosong</p>
        </div>
    </div>
</nav>