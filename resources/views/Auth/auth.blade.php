...
<body class="bg-amber-50 min-h-screen flex items-center justify-center p-4">
    <div x-data="authApp()" class="w-full max-w-md mx-auto">
        ...
        <div class="flex mb-6 md:mb-8">
            ...
            <button 
                @click="currentTab = 'register'; registerStep = 1;"
        ...
        <!-- Login Form -->
        <div x-show="currentTab === 'login'" x-transition class="bg-white p-6 md:p-8 rounded-lg shadow-lg border border-amber-200">
            <h2 class="text-xl md:text-2xl font-bold text-chocolate mb-4 md:mb-6 text-center">Masuk ke Akun Anda</h2>
            
            <form @submit.prevent="login" class="space-y-3 md:space-y-4">
                <div>
        ...
        <!-- Register Form -->
        <div x-show="currentTab === 'register'" x-transition class="bg-white p-6 md:p-8 rounded-lg shadow-lg border border-amber-200">
            <h2 class="text-xl md:text-2xl font-bold text-chocolate mb-4 md:mb-6 text-center">Buat Akun Baru</h2>
            
            <!-- Progress Bar -->
            <div class="progress-container">
                <div class="progress-bar bg-chocolate" :style="'width: ' + (registerStep === 1 ? '50%' : '100%')"></div>
            </div>
            
            <!-- Step 1: Basic Info -->
            <form 
                x-show="registerStep === 1" 
                @submit.prevent="nextStep" 
                class="space-y-3 md:space-y-4"
            >
                <div>
                    <label for="register-name" class="block text-sm font-medium text-amber-900 mb-1">Nama Lengkap</label>
                    <input 
                        type="text" 
                        id="register-name" 
                ...
                <div>
                    <label for="register-password" class="block text-sm font-medium text-amber-900 mb-1">Password</label>
                    <div class="relative">
                        <input 
                ...
                <div>
                    <label for="register-confirm-password" class="block text-sm font-medium text-amber-900 mb-1">Konfirmasi Password</label>
                    <div class="relative">
                        <input 
                            :type="showRegisterPasswords ? 'text' : 'password'" 
                            id="register-confirm-password" 
                            x-model="registerData.confirmPassword"
                            required
                            minlength="6"
                            class="w-full px-3 md:px-4 py-2 rounded-lg border border-amber-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition text-sm md:text-base"
                            placeholder="••••••••"
                        ...
                        <button 
                            ...
                            class="absolute right-3 top-1/2 transform -translate-y-1/2 text-amber-700 hover:text-amber-900"
                        >
                            <i :class="showRegisterPasswords ? 'fa-eye-slash' : 'fa-eye'" class="fas text-sm md:text-base"></i>
                        </button>
                    </div>
                    <p x-show="registerData.password && registerData.confirmPassword && registerData.password !== registerData.confirmPassword" class="text-red-500 text-xs mt-1">Password tidak cocok</p>
                </div>
                
                <button 
                    type="submit" 
                    :disabled="registerData.password && registerData.confirmPassword && registerData.password !== registerData.confirmPassword"
                    :class="{'opacity-50 cursor-not-allowed': registerData.password && registerData.confirmPassword && registerData.password !== registerData.confirmPassword}"
                    class="w-full bg-chocolate text-white py-2 px-4 rounded-lg hover:bg-amber-800 transition-colors duration-300 font-medium text-sm md:text-base"
                >
                    Selanjutnya
                </button>
            ...
            >
                ...
                <div>
                    <label for="register-username" class="block text-sm font-medium text-amber-900 mb-1">Username</label>
                    <input 
                        type="text" 
                ...
                <div>
                    <label for="register-class" class="block text-sm font-medium text-amber-900 mb-1">Kelas</label>
                    <select 
                        id="register-class" 
                        x-model="registerData.class"
                        required
                ...
                <div>
                    <label for="register-phone" class="block text-sm font-medium text-amber-900 mb-1">Nomor Telepon</label>
                    <input 
                        type="tel" 
                        id="register-phone" 
                ...
                <div class="flex items-start">
                    ...
                    <label for="terms" class="ml-2 block text-xs md:text-sm text-amber-800">
                        Saya menyetujui <a href="#" class="text-amber-700 hover:text-amber-900 font-medium">Syarat & Ketentuan</a>
                    </label>
                </div>
